...
function on_primary() { printf "${PURPLE}On primary: ${NC}$@\n"; ssh -l admin -p 2024 -o LogLevel=ERROR -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ${NSO_VIP} "$@" ; }
function on_node() { printf "${PURPLE}On $1: ${NC}$2\n"; ssh -l admin -p 2024 -o LogLevel=ERROR -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no "$1" "$2" ; }

function on_primary_sh() { printf "${PURPLE}On primary: ${NC}$@\n"; ssh -l admin -p 22 -o LogLevel=ERROR -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ${NSO_VIP} "$@" ; }
function on_node_sh() { printf "${PURPLE}On $1: ${NC}$2\n"; ssh -l admin -p 22 -o LogLevel=ERROR -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no "$1" "$2" ; }
...
on_primary "high-availability disable"
...
on_primary_sh 'rm $NCS_DIR; ln -s $NCS_ROOT_DIR/ncs-$NEW_NSO_VERSION $NCS_DIR'
on_node_sh ${NODE2_NAME} 'rm $NCS_DIR; ln -s $NCS_ROOT_DIR/ncs-$NEW_NSO_VERSION $NCS_DIR'
...
on_node_sh ${NODE1_NAME} '$NCS_DIR/bin/ncs --stop; $NCS_DIR/bin/ncs -c $NCS_CONFIG_DIR/ncs.conf --with-package-reload-force'
...
on_node_sh ${NODE2_NAME} '$NCS_DIR/bin/ncs --stop; $NCS_DIR/bin/ncs -c $NCS_CONFIG_DIR/ncs.conf --with-package-reload-force'
...
on_primary "software packages list"
on_primary "software packages install package inert-1.0; software packages install package dummy-1.1 replace-existing; software packages list"
on_primary "software packages list"
...
printf "\n${PURPLE}Start forwarding port 12024 to the VIP port 2024\n${NC}"
