...
generate_host_certs()
{
    ...
    ( set -x; umask 077 && \
          openssl req -newkey ec -pkeyopt ec_paramgen_curve:secp384r1 \
                  -keyout "/etc-$1/dist/ssl/private/$1.key" -nodes \
                  -out "/ssl/csr/$1.csr" -subj "/CN=$1" ) && \
        ( set -x; openssl x509 -req -CAcreateserial \
                          -in "/ssl/csr/$1.csr" \
                          -CA "/ssl/certs/ca.crt" \
                          -CAkey "/ssl/private/ca.key" \
                          -days 3652 -out "/etc-$1/dist/ssl/certs/$1.crt" )

    if [ $? -ne 0 ]; then
        echo "$scrptname: error: failed to generate host certs/keys for $1" >&2
        exit 1
    fi
    cp  /ssl/certs/ca.crt /etc-$1/dist/ssl/certs/ca.crt
    openssl x509 -noout -text -in /etc-$1/dist/ssl/certs/$1.crt
}
...
generate_tls_certificates()
{
    ...
    for NODE in "${NODES[@]}" ; do
        mkdir -p /etc-$NODE/dist/ssl/certs /etc-$NODE/dist/ssl/private
        chmod 600 /etc-$NODE/dist/ssl/private
