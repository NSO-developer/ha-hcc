...
high-availability settings reconnect-interval 3;
...
printf "\n${PURPLE}##### Observe a failover by bringing down $PRIMARY (current primary) and have it wait to come up until the secondary assume primary role\n${NC}"
set +e
on_primary_sh "touch $NCS_RUN_DIR/upgrade; ncs --stop"

printf "\n${PURPLE}##### The secondary will attempt to reconnect to the primary $RA times every $RI s (timeout after $TIMEOUT s)\n${NC}"
counter=$TIMEOUT
until [ $counter -lt 0 ] ; do
    printf "${RED}##### Waiting for the secondary to timeout and failover to primary $counter\n${NC}"
    ((counter--))
    sleep 1
done
set -e
...
printf "\n${PURPLE}##### Stert the previous primary ${NODE1}\n${NC}"
on_node_sh ${NODE1} 'rm $NCS_RUN_DIR/upgrade'
